...
if use_auto_pricing:
    ...
    # SLEEVE ADJUSTMENT: Add $0.70 to base matrix price if screens > 0
    base_rs = get_price_from_matrix(units, screens_rs)
    cost_rs = (base_rs + 0.70) if base_rs > 0 else 0.0
    
    base_ls = get_price_from_matrix(units, screens_ls)
    cost_ls = (base_ls + 0.70) if base_ls > 0 else 0.0
else:
