...
with st.sidebar:
    ...
    blank_price = st.number_input("Blank Garment Cost ($)", value=3.33, help="Find prices on [ACC](https://www.orderacc.com)")
    ...
    if is_reorder:
        calculated_screen_price = 0.0 if units > 144 else 15.0
        lock_msg = "Reorder >144 pcs: Free" if units > 144 else "Reorder <145 pcs: $15"
    else:
...
with retail_container:
    ...
    if use_suggested:
        ...
        if optimization_strategy == "Contribution Margin Only":
            target_cm_percent = st.number_input("Target Contribution Margin (%)", value=25.0, step=1.0, format="%.1f")
        ...
        min_retail_cm = 999.00 if cm_denom <= 0 else (total_cogs / cm_denom) * 2
        
        if optimization_strategy == "Contribution Margin Only":
            target_retail = min_retail_cm
            help_text = f"Optimized for CM > {target_cm_percent}% only"
        else:
            target_retail = max(min_retail_gm, min_retail_cm)
            help_text = f"Calculated for GM > {target_gm_percent}% AND CM > {target_cm_percent}%"
            
        suggested_retail = math.ceil(target_retail / 0.05) * 0.05
        
        retail_price = st.number_input("Suggested Retail Price ($)", value=float(round(suggested_retail, 2)), disabled=True, help=help_text)
    else:
...
c1.markdown(f"**Unit Wholesale Price**<br><span style='font-size:32px'>${wholesale_price:,.2f}</span>", unsafe_allow_html=True)
c2.markdown(f"**Total Wholesale Price**<br><span style='font-size:32px'>${total_wholesale_revenue:,.2f}</span>", unsafe_allow_html=True)
c3.markdown(f"**Gross Margin**<br><span style='font-size:32px'>${gross_margin_dollar:,.2f}</span> <span style='color:{gm_color}; font-size:20px'>({gross_margin_percent:.1f}%)</span>", unsafe_allow_html=True)
c4.markdown(f"**Contrib. Margin**<br><span style='font-size:32px'>${contribution_margin_dollar:,.2f}</span> <span style='color:{cm_color}; font-size:20px'>({contribution_margin_percent:.1f}%)</span>", unsafe_allow_html=True)
...
for cm_target in scenario_cms:
    ...
    scenario_data.append({"CM Goal": f"{cm_target}%", "Rec. Unit Wholesale": rec_wholesale, "Rec. Total Wholesale": rec_total})

df_scenarios = pd.DataFrame(scenario_data)
st.dataframe(df_scenarios.style.format({"Rec. Unit Wholesale": "${:,.2f}", "Rec. Total Wholesale": "${:,.2f}"}), use_container_width=True, hide_index=True)
...
chart_data = pd.DataFrame({
    ...
    "Value": [
        blank_price, 
        (final_print_cost_per_unit + vas_cost + fleece_charge),
        var_expenses_dollar,
        contribution_margin_dollar
    ]
})

fig = px.pie(chart_data, values="Value", names="Cost Component", hole=0.4, color_discrete_sequence=px.colors.qualitative.Pastel)
...
st.dataframe(df.style.format({"Per Unit": "${:.2f}", "Total Run": "${:,.2f}"}), use_container_width=True, hide_index=True)
...
with st.expander("Show detailed breakdown"):
    ...
    # NEW LOGIC: Print cost sans flash
    base_print_cost = raw_print_cost_per_unit - total_flash
    st.write(f"**Pre-Discount Print Per Unit Cost:** ${base_print_cost:.2f}")
